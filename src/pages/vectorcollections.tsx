  ...
  const columns: ColumnDef<UIVectorCollection>[] = [
    {
      ...
      render: (row) => (
        <div className="tbl-cell-user">
          <img src={getModelIcon('amazon')} alt={row.name} />
          <div className="user-info">
            <span className="primary-text">{row.name}</span>
            <span className="secondary-text">{row.documents?.length ?? 0} Multimodal {row.documents?.length === 1 ? 'Document' : ' Documents'}</span>
          </div>
        </div>
      )
    },
    ...
    },
    {
      ...
      sortable: true,
      render: (row) => (
        <span className="tbl-badge info">{row.embeddingModel}</span>
      )
    },
    {
      header: 'Actions',
  ...
  return (
    <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', minWidth: 0, overflowX: 'hidden' }}>
            ...
            <div style={{ 
              ...
              <div style={{ marginBottom: '1rem' }}>
                <span style={{ display: 'block', fontSize: '0.75rem', color: darkMode ? '#9ca3af' : '#6b7280' }}>Ingestion Engine Strategy</span>
      ...
      >
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '2rem' }}>
          ...
          <div style={{ 
            ...
            <h4 style={{ margin: 0, fontSize: '0.875rem', color: '#10b981', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 700 }}>
              Automated Multi-Modal Routing
            </h4>
            <p style={{ margin: 0, fontSize: '0.875rem', color: darkMode ? '#d1d5db' : '#4b5563', lineHeight: 1.5, textAlign: 'justify' }}>
              Vector Collections utilize a Federated Embedding Architecture Matrix that automatically handles embedding model selection and mathematical vector sizes.
            </p>
            <div style={{ fontSize: '0.825rem', color: darkMode ? '#9ca3af' : '#6b7280', display: 'flex', flexDirection: 'column', gap: '0.5rem', textAlign: 'justify' }}>
              <div>• <strong>Documents (.pdf, .txt, .csv):</strong> Processed via Amazon Titan Text Embeddings V2 with Hierarchical token chunk tracking.</div>
              <div>• <strong>Media (.png, .jpeg, .mp4):</strong> Processed via Amazon Nova Multimodal Embeddings extraction operations.</div>
            </div>
          </div>
        </div>
      </ExtraLargeModal>
      ...
      <FullScreenModal
        ...
        title={editVectorCollection ? `Manage Storage Matrix: ${editVectorCollection.name}` : 'Edit Collection'}
        darkMode={darkMode}
        footer={
      ...
      >
        <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '2rem', height: '100%' }}>
          <div style={{ 
            display: 'flex', flexDirection: 'column', gap: '1.5rem', paddingRight: '2rem', 
            borderRight: `1px solid ${darkMode ? '#374151' : '#e5e7eb'}`, overflowY: 'auto', minWidth: 0, overflowX: 'hidden'
          }}>
            <div>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 500, color: darkMode ? '#ccc' : '#333' }}>Collection Container Name</label>
              <input 
                type="text" 
                name="name"
            ...
            <div style={{ backgroundColor: darkMode ? '#1f2937' : '#f9fafb', padding: '1rem', borderRadius: '0.5rem', border: `1px solid ${darkMode ? '#374151' : '#e5e7eb'}` }}>
              <h4 style={{ margin: '0 0 0.75rem', fontSize: '0.825rem', color: darkMode ? '#9ca3af' : '#6b7280', textTransform: 'uppercase' }}>Pipeline Engine</h4>
              <span style={{ fontSize: '0.875rem', color: darkMode ? '#f9fafb' : '#111827', fontWeight: 600 }}>{editVectorCollectionData.embeddingModel}</span>
            </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.875rem' }}>
                ...
                <tbody>
                  {vectorDocuments.length === 0 ? (
                    <tr>
                      <td colSpan={4} style={{ padding: '3rem 1rem', textAlign: 'center', color: darkMode ? '#9ca3af' : '#6b7280', fontSize: '0.875rem' }}>
      ...
      <BottomRightModal
        ...
        footer={
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
            <button className="bottom-right-modal-button" onClick={() => setIsDeleteModalOpen(false)} style={{ padding: '0.5rem 1rem', cursor: 'pointer' }}>
              Cancel
            </button>
            <button 
