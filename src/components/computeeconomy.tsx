...
const ComputeEconomy: React.FC<ComputeEconomyProps> = ({ searchQuery, darkMode }) => {
    ...
    // --- 1. Lollipop Chart (Top Tool Expenditure) ---
    useEffect(() => {
    ...
    // --- 3. Parallel Coordinates (Execution Pathing) ---
    useEffect(() => {
    ...
    // --- 4. Radar Chart (Model Caliber Comparison) ---
    useEffect(() => {
        ...
        // FIX: Replaced (d, i) with (_, i) for unused parameters
    ...
    // --- 5. Dendrogram (Ecosystem Taxonomy) ---
    useEffect(() => {
    ...
    // --- 6. Choropleth/Node Map (Regional Compute Burn) ---
    useEffect(() => {
